! function(t, e) {
    ...
    const e = (t, ...i) => {
            ...
            for (let n = 0; n < s; n++) {
                ...
                Object.entries(s).forEach((([i, s]) => {
                    ...
                    t[i] || Object.assign(t, {
                        [i]: n
                    }), "object" == typeof(o = s) && null !== o && o.constructor === Object && "[object Object]" === Object.prototype.toString.call(o) ? Object.assign(t[i], e(n, s)) : Array.isArray(s) ? Object.assign(t, {
                        [i]: [...s]
                    }) : Object.assign(t, {
                        [i]: s
                    })
    ...
    const c = (t, e = 1e4) => (t = parseFloat(t + "") || 0, Math.round((t + Number.EPSILON) * e) / e),
        u = {
            classes: {
    ...
    class d extends h {
        ...
        onClickModern(t) {
            ...
            o < 0 && (o = n(t.clientX + this.thumbGap, t.clientY), o === i && (o = i - 1)), o < 0 && (o = n(t.clientX - this.thumbGap, t.clientY), o === i && (o = i + 1)), o < 0 && (o = (e => {
                let s = n(t.clientX - e, t.clientY),
                    r = n(t.clientX + e, t.clientY);
                return o < 0 && s === i && (o = i + 1), o < 0 && r === i && (o = i - 1), o
        ...
        onTransformM() {
            ...
            o(e, this.cn("isResting"), s.state !== l.Init && s.isResting);
            ...
            for (const e of t.slides) {
                let i = e.index,
                    s = e.thumbSlideEl;
                if (!s) continue;
                o(s, this.cn("isSelected"), i === t.page), c = 1 - Math.abs(t.getProgress(i)), s.style.setProperty("--progress", c ? c + "" : "");
                const l = .5 * ((e.thumbWidth || 0) - a);
                h += n, h += l, c && (h -= c * (l + r)), s.style.setProperty("--shift", h - n + ""), h += l, c && (h -= c * (l + r)), h -= n, 0 === i && (u = r * c)
            }
            i && (i.style.setProperty("--left", u + ""), i.style.setProperty("--width", h + u + n + r * c + "")), this.shouldCenter && this.centerModern()
        }
        buildClassic() {
            const {
                container: t,
                track: i
            } = this, s = this.getSlides();
            if (!t || !i || !s) return;
        ...
        buildModern() {
            ...
            const {
                ...
                track: e,
                instance: s
            } = this, o = this.option("thumbTpl") || "";
            if (!t || !e || !o) return;
            n(t, "is-horizontal"), this.updateModern();
            for (const t of s.slides || []) {
                const s = document.createElement("div");
                if (n(s, this.cn("slide")), t.type) {
                    let e = `for-${t.type}`;
                    ["video", "youtube", "vimeo", "html5video"].includes(t.type) && (e += " for-video"), n(s, e)
                }
                s.appendChild(i(this.formatThumb(t, o))), t.thumbSlideEl = s, e.appendChild(s), this.resizeModernSlide(t)
            }
            const r = new s.constructor.Panzoom(t, {
                content: e,
                lockAxis: "x",
                zoom: !1,
                panOnlyZoomed: !1,
                bounds: () => {
                    let t = 0,
                        e = 0,
                        i = s.slides[0],
                        n = s.slides[s.slides.length - 1],
                        o = s.slides[s.page];
                    return i && n && o && (e = -1 * this.getModernThumbPos(0), 0 !== s.page && (e += .5 * (i.thumbWidth || 0)), t = -1 * this.getModernThumbPos(s.slides.length - 1), s.page !== s.slides.length - 1 && (t += (n.thumbWidth || 0) - (o.thumbWidth || 0) - .5 * (n.thumbWidth || 0))), {
                        x: {
                            min: t,
                            max: e
                        },
            ...
            r.on("touchStart", ((t, e) => {
                this.shouldCenter = !1
            })), r.on("click", ((t, e) => this.onClickModern(e))), r.on("ready", (() => {
                this.centerModern(), this.emit("ready")
            })), r.on(["afterTransform", "refresh"], (t => {
                this.lazyLoadModern()
            })), this.panzoom = r, this.refreshModern()
        }
        updateModern() {
            if ("modern" !== this.type) return;
            const {
        ...
        centerModern(t) {
            ...
            for (let t = e.page - 3; t < e.page + 3; t++) {
                ...
                i > 0 && i < 1 && (r += i * (this.getModernThumbPos(t) - o))
            }
            let a = 100;
            void 0 === t && (t = .2, e.inTransition.size > 0 && (t = .12), Math.abs(-1 * s.current.e - r) > s.containerRect.width && (t = .5, a = 0)), s.options.maxVelocity = a, s.applyChange({
                panX: c(-1 * r - s.target.e, 1e3),
        ...
        lazyLoadModern() {
            ...
            let o = this.getModernThumbPos(t.page);
            if (e.state !== l.Init || 0 === o)
                for (const o of t.slides || []) {
                    const t = o.thumbSlideEl;
                    if (!t) continue;
                    const r = t.querySelector("img[data-lazy-src]"),
                        a = o.index,
                        h = this.getModernThumbPos(a),
                        l = s - .5 * e.containerRect.innerWidth,
                        c = l + e.containerRect.innerWidth;
                    if (!r || h < l || h > c) continue;
                    let u = r.dataset.lazySrc;
                    if (!u || !u.length) continue;
                    if (delete r.dataset.lazySrc, r.src = u, r.complete) continue;
                    ...
                    t.appendChild(d), r.addEventListener("load", (() => {
        ...
        getModernThumbPos(t) {
            ...
            return o -= n > s ? .5 * s : .5 * n, c(o || 0, 1)
        }
        build() {
            const e = this.instance,
                i = e.container,
                s = this.option("minCount") || 0;
            if (s) {
                let i = 0;
            ...
            if (["modern", "classic"].indexOf(o) < 0) return void(this.state = t.States.Disabled);
            this.type = o;
            const r = document.createElement("div");
            n(r, this.cn("container")), n(r, `is-${o}`);
            const a = this.option("parentEl");
            a ? a.appendChild(r) : i.after(r), this.container = r, n(i, this.cn("hasThumbs"));
            const h = document.createElement("div");
            n(h, this.cn("track")), r.appendChild(h), this.track = h, "classic" === o ? this.buildClassic() : this.buildModern(), this.state = t.States.Ready, r.addEventListener("click", (t => {
                setTimeout((() => {
                    var t;
                    null === (t = null == r ? void 0 : r.querySelector(`[data-carousel-index="${e.page}"]`)) || void 0 === t || t.focus()
                }), 100)
            }))
        }
        cleanup() {
            var e, i;
            this.carousel && this.carousel.destroy(), this.carousel = null, this.panzoom && this.panzoom.destroy(), this.panzoom = null, this.container && this.container.remove(), this.container = null, this.track = null, this.state = t.States.Init, e = this.instance.container, i = this.cn("hasThumbs"), e && s(i).forEach((t => {
